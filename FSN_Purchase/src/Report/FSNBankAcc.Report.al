report 50012 "FSN Bank Acc Recon"
{
    DefaultLayout = RDLC;
        ...
        dataitem("Bank Acc. Reconciliation"; "Bank Acc. Reconciliation")
        {
            ...
            dataitem(PageCounter; "Integer")
            {
                ...
                /*dataitem(BankLedgerEntry; "Bank Account Ledger Entry")
                {
                    ...
                    column(BankLedgerEntry_EntryNo; "Entry No.")
                    {
                    }
                    column(BankLedgerEntry_Amount; Amount)
                    {
                    }
                    column(BankLedgerEntry_PostingDate; Format("Posting Date"))
                    {
                    }
                    column(BankLedgerEntry_DocType; "Document Type")
                    {
                    }
                    column(BankLedgerEntry_DocNo; "Document No.")
                    {
                    }
                    column(BankLedgerEntry_BankAccNo; "Bank Account No.")
                    {
                    }
                    column(BankLedgerEntry_Description; Description)
                    {
                    }

                    trigger OnAfterGetRecord()
                    begin
                        if "Posting Date" >= "Bank Acc. Reconciliation Line"."Transaction Date" then
                            CurrReport.Skip();
                        if not Open then
                            CurrReport.Skip();
                    end;
