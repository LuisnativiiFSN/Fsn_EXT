report 50007 "FSN Cheque"
{
    DefaultLayout = RDLC;
    RDLCLayout = 'src/Report/Layout/FSNCheque.rdl';
    Caption = 'FSN Cheque';
    ...
    dataset
    {
        ...
        dataitem(GenJnlLine; "Gen. Journal Line")
        {
            ...
            dataitem(CheckPages; "Integer")
            {
                ...
                column(CuentaText; PrnChkLine[1, 1])
                {
                }
                column(CuentaText2; PrnChkLine[2, 1])
                {
                }
                column(NombreText; PrnChkLine[1, 2])
                {
                }
                column(NombreText2; PrnChkLine[2, 2])
                {
                }
                column(MontoText; PrnChkLine[2, 3])
                {
                }
                column(CheckDateTxt; CheckDate)
                {
                }
                ...
                dataitem(PrintSettledLoop; "Integer")
                {
                    ...
                    column(NetAmount; NetAmountLbl)//1690
                    {
                    }
                    column(AmounTextL; AmounTextL)//15837
                    {
                    }
                    column(Descripcion1; Descripcion)
                    {
                    ...
                    begin
                        if not TestPrint then begin
                                ...
                                case ApplyMethod of
                                    ApplyMethod::OneLineOneEntry:
                                        begin
                                            ...
                                            CurrentLineAmount := LineAmount2;
                                            FoundLast := true;
                                            AmounTextL := FSNUTILITY.Num2Text(CurrentLineAmount) + '***';
                                        end;
                                    ...
                                    ApplyMethod::MoreLinesOneEntry:
                                        begin
                                            ...
                                            if GenJnlLine2."Applies-to Doc. No." = '' then begin
                                                ...
                                                AmounTextL := FSNUTILITY.Num2Text(LineAmount) + '***';
                                            end else
                                                case BalancingType of
                            ...
                            if FoundLast then
                                CurrReport.Break();
                            FoundLast := true;
                            DocNo := BankAcc2."Last Check No.";
                            ExtDocNo := ExtDocNo;
                            LineAmount := 108.01;
                            LineDiscount := 0;
                            PostingDesc := '';//160120
                            AmounTextL := FSNUTILITY.Num2Text(108.1) + '***';
                        end;
                    ...
                    begin
                        ...
                        Descripcion := GenJnlLine.Description;
            ...
            begin
                ...
                if not TestPrint then begin
                        ...
                        BalancingType::"G/L Account":
                            begin
                                ...
                                ChkTransMgt.SetCheckPrintParams(
                                  ...
                                  CheckLanguage,
                                  CheckStyle);
                                if BankAccPG.get(BankAcc2."Bank Acc. Posting Group") then
                                    NoReference := BankAccPG."G/L Account No."
                                else
                                    NoReference := BankAcc2."Bank Account No.";
                            end;
                        BalancingType::Customer:
                            begin
                        ...
                        BalancingType::"Bank Account":
                            begin
                                ...
                                ChkTransMgt.SetCheckPrintParams(
                                  ...
                                  CheckLanguage,
                                  CheckStyle);
                                if BankAccPG.get(BankAcc."Bank Acc. Posting Group") then
                                    NoReference := BankAccPG."G/L Account No."
                                else
                                    NoReference := BankAcc."Bank Account No.";
                            end;
                        BalancingType::Employee:
                            ApplyBalancingTypeOfEmployee;
                    end;
                    if BankAcc."Bank Account No." <> '' then begin
                        //CuentaText[1] := BankAcc."Bank Account No." + '  ' + BankAcc.Name + '  ' + Format(GenJnlLine.Amount);
                        PrnChkLine[1, 1] := NoReference;
                        PrnChkLine[1, 2] := CheckToAddr[1];
                        PrnChkLine[1, 3] := Format(GenJnlLine.Amount);
                    end else begin
                        //CuentaText[2] := BankAcc2."Bank Account No." + '  ' + BankAcc2.Name + '  ' + Format(GenJnlLine.Amount);
                        PrnChkLine[1, 1] := NoReference;
                        PrnChkLine[1, 2] := CheckToAddr[1];
                        PrnChkLine[1, 3] := Format(GenJnlLine.Amount);
                    end;
                    ...
                    CheckDate := "Posting Date";//'S.S ' + DelChr(ChkTransMgt.FormatDate("Posting Date", CheckDateFormat, DateSeparator, CheckLanguage, DateIndicator), '=', 'de');
                    ...
                    CheckDateFormat := CheckDateFormat::" ";
                    CheckDate := "Posting Date";//'S.S ' + DelChr(ChkTransMgt.FormatDate("Posting Date", CheckDateFormat, DateSeparator, CheckLanguage, DateIndicator), '=', 'de');
                    IF BankAcc.Get(GenJnlLine."Bill-to/Pay-to No.") THEN;
                    //CuentaText[1] := BankAcc."Bank Account No." + ' ' + BankAcc.Name + ' ' + Format(GenJnlLine.Amount);
                    //CuentaText[2] := BankAcc2."Bank Account No." + ' ' + BankAcc2.Name + ' ' + Format(GenJnlLine.Amount);

                    PrnChkLine[1, 1] := 'PROV-000314';
                    PrnChkLine[1, 2] := 'FREUND DE EL SALVADOR LTDA DE C.V.';
                    PrnChkLine[1, 3] := Format(GenJnlLine.Amount);
    ...
    var
        ...
        Vend: Record Vendor;
        CuentaText: array[6] of Text;
        DescripcionText: array[6] of Text;
        ...
        NoReference: Text[30];
        CheckDate: Date;
        PrnChkLine: array[2, 3] of Text[80];
