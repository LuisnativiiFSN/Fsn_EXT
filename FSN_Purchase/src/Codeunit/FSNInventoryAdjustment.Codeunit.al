codeunit 50065 "FSN Inventory Adjustment"
{
    ...
    begin
        ...
        while (InvtToAdjustExist(TempItem) or IsFirstTime) and not LevelExceeded do begin
            ...
            //OnMakeMultiLevelAdjmtOnAfterMakeAdjmt(TempAvgCostAdjmtEntryPoint, FilterItem, RndgResidualBuf, IsOnlineAdjmt, PostToGL, ItemJnlPostLine);
            IsFirstTime := false;
        end;
    ...
    begin
        ...
        if AvgCostAdjmtEntryPoint.FindSet() then
            repeat
                TempAvgCostAdjmtEntryPoint.Copy(AvgCostAdjmtEntryPoint);
                TempAvgCostAdjmtEntryPoint."Item No." := AvgCostAdjmtEntryPoint."Item No.";
                TempAvgCostAdjmtEntryPoint."Variant Code" := AvgCostAdjmtEntryPoint."Variant Code";
                TempAvgCostAdjmtEntryPoint."Location Code" := AvgCostAdjmtEntryPoint."Location Code";
    ...
    begin
        ...
        while AvgCostAdjmtEntryPointExist(TempAvgCostAdjmtEntryPoint) do begin
            repeat
                ...
                while not Restart and AvgValueEntriesToAdjustExist(TempOutbndValueEntry, TempExcludedValueEntry, AvgCostAdjmtEntryPoint) and not EndOfValuationDateReached
                do begin
                    ...
                    if AvgCostAdjmtEntryPoint.FindFirst then
                        if not AvgCostAdjmtEntryPoint."Cost Is Adjusted" then
                            Restart := true;
                    //OnAdjustItemAvgCostOnAfterCalcRestart(TempExcludedValueEntry, Restart);
                    if AvgCostAdjmtEntryPoint."Valuation Date" >= PeriodFormMgt.EndOfPeriod() then
    ...
    begin
        ...
        with NewCostElementBuf do begin
            //UpDateWindow(WindowAdjmtLevel, WindowItem, WindowAdjust, WindowFWLevel, OutbndValueEntry."Item Ledger Entry No.", WindowOutbndEntry);
